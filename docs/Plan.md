...
### Негативные сценарии:
Nonexistent карта — 1111 2222 3333 4455

15numbers карта — 1111 2222 3333 444

1. Отправка заявки через сервис **Payment Gate** с заполением поля **Номер карты** Nonexistent номером карты и валидными данными в остальных полях

   *Ожидаемый результат*: отправка формы, появление попапа с текстом об отказе в проведении операции


2. Отправка заявки через сервис **Credit Gate** с заполением поля **Номер карты** Nonexistent номером карты и валидными данными в остальных полях

   *Ожидаемый результат*: отправка формы, появление попапа с текстом об отказе в проведении операции


3. Отправка заявки через сервис **Payment Gate** с заполением поля **Номер карты** 15numbers номером карты и валидными данными в остальных полях

   *Ожидаемый результат*: форма не отправляется, под полем ввода **Номер карты** появляется сообщение об ошибке в веденных данных


4. Отправка заявки через сервис **Credit Gate** с заполением поля **Номер карты** 15numbers номером карты и валидными данными в остальных полях
